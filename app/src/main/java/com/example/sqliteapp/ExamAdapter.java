...
public class ExamAdapter extends ArrayAdapter<Exam> {
    ...
    public ExamAdapter(Context context, int listLayoutRes, List<Exam> examsList, SQLiteDatabase myDb) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        //getting views
        TextView examName = view.findViewById(R.id.examNameTV);
        ...
        buttonKey.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String sql = "select "+ DatabaseHelper.COL_3 +" from "+ DatabaseHelper.TABLE_NAME+" where id = ?";
                Cursor cursor = myDb.rawQuery("select "+ DatabaseHelper.COL_3 +" from "+ DatabaseHelper.TABLE_NAME+" where id = "+ exam.getId(), null);

                if (cursor.moveToFirst()) {
                    do {
                        String key = cursor.getString(0);
                        Toast.makeText(context, key, Toast.LENGTH_LONG).show(); // tutaj przekazanie klucza do innej aktywności/widoku
